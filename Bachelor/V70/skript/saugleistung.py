...
plt.errorbar(506 , 3.8, xerr = np.sqrt(2)*504,    yerr=0.4, fmt='.', label = "Evakuierung 1")
plt.errorbar(1.25,1.35, xerr = np.sqrt(2)*0.755,  yerr=0.16,fmt='.', label = "Evakuierung 2")
plt.errorbar(0.53,0.53, xerr = np.sqrt(2)*0.165,  yerr=0.06,fmt='.', label = "Evakuierung 3")
plt.xscale('log')
plt.xlabel(r"$p$ $ [mbar]$")
...
plt.errorbar(0.0303, 5,  xerr = np.sqrt(2)*0.00667, yerr=8,    fmt='.', label = "Evakuierung 3")
plt.errorbar(0.0186, 1.2,xerr = np.sqrt(2)*0.00507, yerr=0.8,  fmt='.', label = "Evakuierung 4")

plt.xscale('log')
plt.xlabel(r"$p$ $ [10^{-3}mbar]$")
plt.ylabel(r"$S$ $ [\frac{m³}{h}]$")
plt.tight_layout()
plt.legend(loc = 'best')
